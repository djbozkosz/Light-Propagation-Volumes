...
void main()
{
  vec2 p = gl_in[0].gl_Position.xy; // zeros
  p += vec2(float(vertexID[0] % int(fragSize.x)), float(vertexID[0] / int(fragSize.y))) * fragSize.zw;
  ...
  if((pos.x != 0.0) && (pos.y != 0.0) && (pos.z != 0.0) && (l.x != 0.0) && (l.y != 0.0) && (l.z != 0.0))
  {
    ...
    pos = vec3(pos.xy / (lpvTexSize.xy * 0.5), clamp(pos.z + lpvTexSize.z * 0.5, 0.0, lpvTexSize.z));
